 ...
 */
@AllArgsConstructor
public class NodeContainer implements PersistentResourceContainer, GraphQLContainer<CompletableFuture<Object>> {
    @Getter protected final PersistentResource<?> persistentResource;

    @Override
    public CompletableFuture<Object> processFetch(Environment context) {
        ...
        return CompletableFuture.supplyAsync(() -> {
            if (entityDictionary.isAttribute(parentClass, fieldName)) { /* fetch attribute properties */
                Attribute requested = context.requestScope.getProjectionInfo().getAttributeMap()
                ...
                if (attribute instanceof Map) {
                    return ((Map<Object, Object>) attribute).entrySet().stream().map(MapEntryContainer::new).toList();
                }
                ...
                if (attribute instanceof Collection) {
                    ...
                    if (nonEntityDictionary.hasBinding(innerType)) {
                        return ((Collection) attribute).stream().map(NonEntityContainer::new).toList();
                    }
                }
            ...
            if (entityDictionary.isRelation(parentClass, fieldName)) { /* fetch relationship properties */
                ...
                Relationship relationship = context.requestScope.getProjectionInfo().getRelationshipMap()
                        .getOrDefault(context.field.getSourceLocation(), null);
            ...
            throw new BadRequestException(
                    "Unrecognized object: " + fieldName + " for: " + parentClass.getName() + " in node");
        });
    }
